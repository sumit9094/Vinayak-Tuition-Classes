...
import { LogOut, Sun, Moon, Globe, GraduationCap } from 'lucide-react';
  ...
  if (isLoading || !user || user.type !== 'student') {
    return (
      <div className="min-h-screen w-full flex flex-col justify-center items-center bg-slate-50 dark:bg-darkObsidian transition-colors duration-300">
        ...
        <p className="mt-4 text-xs font-semibold text-slate-500 dark:text-slate-400">Verifying student session...</p>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-4 sm:px-6 flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center space-x-2 group">
            ...
            <div className="hidden sm:flex bg-[#8B5CF6]/10 text-[#8B5CF6] text-[9px] font-black uppercase px-2 py-0.5 rounded-full items-center border border-[#8B5CF6]/20">
              ...
              {language === 'GJ' ? 'વિદ્યાર્થી પોર્ટલ' : 'Student Portal'}
            </div>
          </Link>
