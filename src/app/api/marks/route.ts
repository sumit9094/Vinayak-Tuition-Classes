      ...
      const pushPromises = records
        ...
        .map((rec: any) =>
          sendPushToUser(String(rec.studentId), 'student', {
            ...
            url: '/student/dashboard?tab=marks'
...
export async function GET(req: Request) {
  try {
    ...
    if (session.role === 'teacher' || session.role === 'admin') {
      ...
      if (standardParam && standardParam !== 'All Standards') {
        allowedStudentQuery.standard = standardParam;
      ...
      // Get all distinct test names for these allowed students, branch, and subject
      const distinctTests = await TestMark.distinct('testName', {
        branch,
        subject,
        studentId: { $in: allowedStudentIds },
      });

      if (testName && testName !== 'All Tests') {
        query.testName = testName;
      }
      ...
      return NextResponse.json({ records, distinctTests });
    }
