...
import { User, Mail, Phone, BookOpen, MapPin, AlertCircle, ArrowRight } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.code === 'DUPLICATE_EMAIL' || data.error?.toLowerCase().includes('already')) {
          setIsDuplicateEmail(true);
          setDuplicateEmailValue(formData.email);
          setErrorBanner(
            language === 'EN'
              ? 'You already have an account with this email — please log in instead.'
              : 'આ ઈમેલ આઈડી સાથે એકાઉન્ટ પહેલેથી રજિસ્ટર થયેલું છે — કૃપા કરીને લોગિન કરો.'
          );
        } else {
          setIsDuplicateEmail(false);
  ...
  return (
    <AuthCard title={t('authRegisterTitle')}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {errorBanner && (
          <div className={`p-3.5 rounded-xl text-xs font-semibold animate-fadeIn space-y-2 ${
            isDuplicateEmail 
            ...
            {isDuplicateEmail && (
              <div className="pt-1 flex justify-end">
                <Link
                  href={`/login?email=${encodeURIComponent(duplicateEmailValue)}`}
                  className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-lg bg-[#8B5CF6] hover:bg-[#7C3AED] text-white font-bold text-xs transition-all shadow-sm cursor-pointer"
                >
                  <span>{language === 'EN' ? 'Go to Login' : 'લોગિન કરો'}</span>
