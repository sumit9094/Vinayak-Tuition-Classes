...
export default function Header() {
  ...
  const handleInstallClick = async () => {
    if (platform === 'ios') {
      // iPhone/iOS: show guide modal directly (unchanged!)
      setShowModal(true);
    } else {
      // Android / Desktop: prompt native install via beforeinstallprompt
      if (deferredPrompt) {
        try {
        ...
        // Fallback: If native prompt is not available, show manual browser instruction modal
        setShowModal(true);
      }
    }
  };
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-40 bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800 py-3.5 shadow-sm transition-all duration-300">
          ...
          {user ? (
            <div className="flex items-center space-x-2">
              <Link
                ...
                className="w-9 h-9 rounded-full bg-[#8B5CF6] hover:bg-[#7C3AED] text-white font-black text-xs flex items-center justify-center shadow-sm transition-colors"
              ...
              <button
                ...
                className="p-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-500 transition-colors focus:outline-none flex items-center justify-center cursor-pointer"
                title="Logout"
              >
            ...
            <Link 
              href="/login-select" 
              className="px-4 py-2 rounded-xl bg-[#8B5CF6] hover:bg-[#7C3AED] text-white font-black text-xs transition-all shadow-sm flex items-center space-x-1.5 cursor-pointer"
            >
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 md:hidden">
          ...
          {user ? (
            <Link 
              href={user.type === 'student' ? '/student/dashboard' : user.role === 'teacher' ? '/teacher/dashboard' : '/admin/dashboard'}
              className="w-8 h-8 rounded-full bg-[#8B5CF6] text-white font-black text-xs transition-colors focus:outline-none flex items-center justify-center shadow-sm"
            ...
            <Link 
              href="/login-select" 
              className="px-3 py-1.5 rounded-xl bg-[#8B5CF6] hover:bg-[#7C3AED] text-white font-black text-xs transition-all shadow-sm flex items-center space-x-1 cursor-pointer"
        ...
        {isOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed inset-0 bg-slate-950/60 z-[9998] md:hidden"
            />
            ...
            <motion.div 
              ...
              className="fixed top-0 right-0 bottom-0 h-full h-[100dvh] w-[290px] sm:w-[330px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-2xl z-[9999] p-6 flex flex-col justify-between overflow-y-auto md:hidden"
            >
              {/* Top Bar: Logo & Close */}
              <div>
              ...
              <div className="space-y-6 mt-8">
                ...
                {user ? (
                  <div className="space-y-2">
                    ...
                    <button 
                      onClick={() => { setIsOpen(false); handleHeaderLogout(); }}
                      className="w-full text-center py-3.5 rounded-xl border border-red-200 dark:border-red-900/50 text-red-500 font-bold block text-sm tracking-wide hover:bg-red-500/5 transition-colors cursor-pointer"
                    >
                      Logout
                    </button>
                  </div>
