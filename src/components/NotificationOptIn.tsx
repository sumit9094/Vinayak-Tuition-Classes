...
export default function NotificationOptIn() {
  ...
  useEffect(() => {
    ...
    // If permission is already granted or denied, don't show banner
    if (Notification.permission === 'granted' || Notification.permission === 'denied') {
      return;
    }

    // Check if dismissed in current session
    const sessionDismissed = sessionStorage.getItem('push_opt_in_dismissed');
    if (sessionDismissed === 'true') {
      return;
    }

    // Show opt-in banner after a short delay (1.5 seconds)
  ...
  const handleEnable = () => {
    setShowBanner(false);
    sessionStorage.setItem('push_opt_in_dismissed', 'true');
    ...
    <AnimatePresence>
      ...
      >
        <div className="flex items-start justify-between space-x-3">
          <div className="p-1.5 bg-[#8B5CF6]/10 rounded-xl shrink-0">
            <img src="/logo.png" alt="Vinayak Tuition Logo" className="w-7 h-7 object-contain rounded-lg shadow-sm" />
          </div>
